...
class FPNHead(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes):
        ...
        for _ in range(len(in_channels)):
            self.conv1x1.append(nn.Conv2d(in_channels[_], out_channels, kernel_size=1))
            self.upsample.append(nn.Upsample(scale_factor=2 ** _, mode="bilinear", align_corners=True))

    def forward(self, x):
        out = []
        for i, feature_map in enumerate(x):
            out.append(self.upsample[i](self.conv1x1[i](feature_map)))
        out = sum(out)
...
class CrossFormerSegmentation(nn.Module):
    def __init__(
        ...
        *,
        ...
        #     nn.Linear(last_dim, num_classes)
        # )
        # Replace final logits with segmentation layer
        self.segmentation_head = nn.Sequential(
            nn.Conv2d(last_dim, num_classes, kernel_size=1),
    ...
    def forward(self, x):
        ...
        return self.fpn_head(feature_maps)