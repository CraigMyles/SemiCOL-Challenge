...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        # if path contains .ome.tiff, remove both parts
        if ".ome.tiff" in path:
            self.name = ".".join(path.split("/")[-1].split('.')[:-2])
        else:
            self.name = os.path.splitext(os.path.basename(path))[0]
        self.wsi = tifffile.TiffFile(path) # i.e. returns "OpenSlide('/data/OTHER_DATASETS/ShareFTP/single_file_test/CRC_0001.svs')"

        self.level_dim = self.get_level_dimensions() # i.e. returns "((102465, 88889), (25616, 22222), (6404, 5555), (1601, 1388))"
        # print(f"LEVEL DIMS from __init__: {self.level_dim}")

        self.level_downsamples = self._assertLevelDownsamples() # i.e. returns [(1.0, 1.0), (4.000039038101186, 4.0000450004500046), (16.000156152404745, 16.001620162016202), (64.00062460961898, 64.04106628242074)]

        # self.get_best_level_for_downsample = get_best_level_for_downsample()
    ...
    def _assertLevelDownsamples(self):
        level_downsamples = []
        # print(f"\n\n\n Inside _assertLevelDownsamples() \n\n\n")
        dim_0 = self.level_dim[0]
        
        for dim in self.level_dim:
            downsample_ratio = (dim_0[0]/float(dim[0]), dim_0[1]/float(dim[1]))
            level_downsamples.append(downsample_ratio)
        
        # print(level_downsamples)
        return level_downsamples
    
    def get_level_dimensions(self):
        # print(f"\nINSIDE GET_LEVEL_DIMENSIONS\n")
        level_dim = []
        # Open the TIFF file
        with self.wsi as tif:
            # Get the TiffPageSeries object
            series = tif.series[0]
            
            # Loop through all the levels and print their sizes
            for level in series.levels:
                width, height, _ = level.shape
                level_dim.append((width, height))
            
        level_dim = tuple(level_dim)
        # print(f"LEVEL DIMS from get_level_dimensions: {level_dim}")
        return level_dim
    
    def get_best_level_for_downsample(self, downsample):
        # Calculate the average downsample factor for each level
        avg_downsamples = [(d[0] + d[1]) / 2 for d in self.level_downsamples]

        # Find the index of the smallest downsample factor that is greater than or equal to the desired downsample
        best_level = next((i for i, d in enumerate(avg_downsamples) if d >= downsample), len(avg_downsamples) - 1)

        return best_level

    def read_region_tifffile(self, location, level, size):

        # print(f"\n\n inside read_region_tifffile() \n\n Using level {level} \n\n Using size {size} \n\n Using location {location} \n\n")

        x, y = location
        width, height = size
        max_x, max_y = self.level_dim[level]

        # Ensure that the requested region does not go beyond the image dimensions
        ...
        with self.wsi as tif:
            series = tif.series[0]
            page = series.levels[level]
            # print(f"page.shape (aka max dimensions): {page.shape}")
            region = page.asarray()[y:y + height, x:x + width, :3]
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        img = np.array(self.read_region_tifffile((0, 0), seg_level, self.level_dim[seg_level]))
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        img = np.array(self.read_region_tifffile(top_left, vis_level, region_size))
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if not blank_canvas:
            ...
            # img = np.array(self.wsi.read_region(top_left, vis_level, region_size).convert("RGB"))
            img = np.array(self.read_region_tifffile(self, top_left, vis_level, region_size))
        else:
            # use blank canvas
