...
def DrawMapFromCoords(canvas, wsi_object, coords, patch_size, vis_level, indices=None, verbose=1, draw_grid=True):
    ...
    # print('downsample: {}'.format(downsamples))
    if indices is None:
        indices = np.arange(len(coords))
    total = len(indices)
    if verbose > 0:
    ...
    for idx in range(total):
        ...
        # print(f"Index: {idx}, Coordinate: {coord}, Level: {vis_level}")
        # print(f"Requesting patch at with following req: {coord}, {vis_level}, {patch_size}")
        level_width, level_height = wsi_object.level_dim[vis_level]
        # print(f"Level dimensions (width, height): {level_width}, {level_height}")

        coord_transformed = np.ceil(coord / downsamples).astype(np.int32)


        patch = np.array(wsi_object.read_region_tifffile((coord_transformed), vis_level, patch_size))
